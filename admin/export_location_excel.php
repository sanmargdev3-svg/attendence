...
// Build query to fetch employees
$query = "SELECT id, employee_id, name, department, week_off FROM users WHERE role = 'employee' AND status = 'Working' AND location = ?";
$params = [$location];
$types = "s";
...
foreach ($employees_by_dept as $dept => $employees) {
    ...
    foreach ($employees as $emp) {
        ...
        for ($day = 1; $day <= $days_in_month; $day++) {
            $status = getStatusForDate($conn, $emp['id'], $day, $emp['week_off'], $month, $year);
            $col = Coordinate::stringFromColumnIndex($day + 1);
