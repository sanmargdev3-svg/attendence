...
// Get filter values
$filter_month = isset($_POST['month']) ? intval($_POST['month']) : date('m');
...
// Use fetch month/year if not set
$month = $filter_month;
$year = $filter_year;

// CRITICAL CHECK: If employee is resigned and resignation is BEFORE export month, deny export
if ($employee['status'] === 'Resign' && !empty($employee['date_of_exit'])) {
    // Parse the date_of_exit (format: YYYY-MM-DD)
    $resign_parts = explode('-', trim($employee['date_of_exit']));
    
    if (count($resign_parts) === 3) {
        $resign_year = intval($resign_parts[0]);
        $resign_month = intval($resign_parts[1]);
        
        // If resignation happened BEFORE this export month, deny export
        // Example: If resigned March (03) and trying to export April (04) of same year = DENY
        if ($resign_year < $year || ($resign_year === $year && $resign_month < $month)) {
            $_SESSION['export_error'] = "❌ Cannot export! Employee '" . htmlspecialchars($employee['name']) . "' resigned on " . $employee['date_of_exit'] . ". Employee cannot be exported for months after resignation.";
            header("Location: export_monthly.php");
            exit();
        }
    }
...
for ($day = 1; $day <= $days_in_month; $day++) {
    $status = getStatusForDate($conn, $employee['id'], $day, $employee['week_off'], $month, $year, $employee['date_of_exit'] ?? null);
    $col = Coordinate::stringFromColumnIndex($day + 1);
...
for ($day = 1; $day <= $days_in_month; $day++) {
    ...
    $date_obj = date('Y-m-d', mktime(0, 0, 0, $month, $day, $year));
    $is_resigned = false;
    if (!empty($employee['date_of_exit']) && strtotime($date_obj) > strtotime($employee['date_of_exit'])) {
        $is_resigned = true;
    }
    
    if ($is_resigned) {
        $sheet->setCellValue($col . $row, '-'); // Blank for resigned period
    } else {
